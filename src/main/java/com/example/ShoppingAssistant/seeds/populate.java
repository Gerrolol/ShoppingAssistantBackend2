...
public class populate implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (String item : coles) {
            futures.add(CompletableFuture.runAsync(() -> colesService.add(item), executor));
        }
        for (String item : woolies) {
            futures.add(CompletableFuture.runAsync(() -> wooliesService.add(item), executor));
        }
        // for (String item : aldi) {
        //     futures.add(CompletableFuture.runAsync(() -> aldiService.add(item), executor));
        // }
